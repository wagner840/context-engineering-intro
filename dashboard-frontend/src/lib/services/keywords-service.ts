...
class KeywordsService {
   ...
   */
  async createKeyword(keywordData: CreateKeywordData): Promise<KeywordWithRelations> {
    ...
    if (existingKeyword) {
      throw new Error(`Keyword "${keywordData.keyword}" já existe para este blog`)
    }
  ...
  async semanticSearch(params: {
    ...
    try {
      ...
      const { data, error } = await this.supabase.rpc('match_keywords_semantic', {
      ...
      if (error) {
        console.warn('Busca semântica falhou, usando busca por texto:', error.message)
  ...
  async getKeywordStats(blog_id: string): Promise<KeywordStats> {
    ...
    const avgSearchVolume = keywords.reduce((sum, k) => sum + (k.msv || 0), 0) / totalKeywords
    const avgDifficulty = keywords.reduce((sum, k) => sum + (k.kw_difficulty || 0), 0) / totalKeywords
    const avgCpc = keywords.reduce((sum, k) => sum + (k.cpc || 0), 0) / totalKeywords
    ...
    const byCompetition = keywords.reduce((acc, k) => {
  ...
  private async checkKeywordExists(blogId: string, keyword: string): Promise<boolean> {
