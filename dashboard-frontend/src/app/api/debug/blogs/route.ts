...
export async function GET() {
  try {
    ...
    const { data: optemil, error: optemilError } = await supabase
      .from("blogs")
      .select("*")
      .eq("domain", "Optemil.com")
      .single();

    console.log("🤖 Einsof7 blog:", einsof7);
    console.log("📈 Optemil blog:", optemil);

    // Test 4: Check service vs anon key behavior
    const serviceKeyStatus = process.env.SUPABASE_SERVICE_ROLE_KEY
      ? "configured"
      : "missing";
    const anonKeyStatus = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY
      ? "configured"
      : "missing";
    ...
    return NextResponse.json({
      ...
      diagnostics: {
        ...
        database_errors: {
          ...
          optemil_error: optemilError?.message,
        },
        blog_data: {
          all_blogs: allBlogs || [],
          einsof7_found: !!einsof7,
          einsof7_data: einsof7,
          optemil_found: !!optemil,
          optemil_data: optemil,
        },
        api_keys: {
        ...
        rls_analysis: {
          ...
          specific_queries_worked: !einsof7Error && !optemilError,
          recommendation:
            totalBlogs === 0
              ? "No blogs found - check if data was inserted properly"
              : einsof7Error || optemilError
