...
export async function POST(request: NextRequest) {
  try {
    ...
    const blogType = site_url.includes('einsof7.com') ? 'einsof7' : 
                     site_url.includes('opetmil.com') ? 'opetmil' : null

    if (!blogType) {
      return NextResponse.json({ error: 'Unsupported site URL' }, { status: 400 })
    }

    // Get blog info from Supabase
    const domain = blogType === 'einsof7' ? 'einsof7.com' : 'opetmil.com'
    ...
    if (action === 'post_published' || action === 'post_updated') {
      ...
      if (deleteError) {
        ...
        return NextResponse.json({ error: 'Failed to mark post as deleted' }, { status: 500 })
      }
