...
export const DASHBOARD_QUERY_KEYS = {
  ...
  executiveBlog: (blogName: string) => [...DASHBOARD_QUERY_KEYS.executive(), blogName] as const,
} as const
...
export function useExecutiveDashboard(blogName?: string) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<ExecutiveDashboard[]> => {
      ...
      const { data, error } = await query.order('blog_name', { ascending: true })
...
export function useBlogNiches() {
  ...
  const niches = executiveDashboard?.reduce((acc, blog) => {
    ...
    subscribe: () => {
      const channel = supabase
        ...
        .on(
          ...
          () => {
            queryClient.invalidateQueries({ queryKey: DASHBOARD_QUERY_KEYS.executive() })
          }
        )
        .on(
          'postgres_changes',
