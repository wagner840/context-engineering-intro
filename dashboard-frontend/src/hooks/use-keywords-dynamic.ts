  ...
  return useMutation({
    ...
    onSuccess: (newKeyword) => {
      ...
      queryClient.setQueryData(
        [QUERY_KEYS.KEYWORD, newKeyword.id], 
        newKeyword
      )
...
export function useKeywordsWithFilters(blogId: string) {
  ...
  const refetchKeywords = useCallback((filters: KeywordFilters) => {
    return queryClient.fetchQuery({
      queryKey: [QUERY_KEYS.KEYWORDS, { ...filters, blog_id: blogId }],
      queryFn: () => keywordsService.getKeywords({ ...filters, blog_id: blogId }),
      staleTime: 0 // Force refresh
    })
  }, [queryClient, blogId])

  const prefetchKeyword = useCallback((id: string) => {
...
export function useKeywordCache() {
  ...
  const refreshKeywordStats = useCallback((blogId: string) => {
    queryClient.invalidateQueries({ 
      queryKey: [QUERY_KEYS.KEYWORD_STATS, blogId] 
    })
  }, [queryClient])

  const preloadKeywords = useCallback(async (blogId: string) => {
    try {
...
export function useOptimisticKeywordUpdate() {
  ...
  const updateKeywordOptimistically = useCallback((
    keywordId: string,
    updates: Partial<KeywordWithRelations>
  ) => {
...
export function useKeywordAnalytics(blogId: string) {
  ...
  const analytics = {
    ...
    averageOpportunityScore: opportunities?.length 
      ? opportunities.reduce((sum, opp) => sum + (opp.opportunity_score || 0), 0) / opportunities.length
      : 0,
