...
// TODO: Atualizar quando production_pipeline for adicionado aos tipos
// type ProductionPipelineItem = Database['public']['Views']['production_pipeline']['Row']
type ProductionPipelineItem = {
...
interface ContentState {
  ...
  schedulePost: (id: string) => Promise<void>;
  ...
  devtools(
    (set, get) => ({
      ...
      fetchPipelineItems: async (blogId: string) => {
        set({ loading: true, error: null });
        try {
          // Query alternativa usando content_posts diretamente
          ...
          const pipeline: ProductionPipelineItem[] = (pipelineData || []).map(
            (post) => ({
              blog_name: null,
              title: post.title,
              status: post.status,
              author_name: (post as any).author?.name || null,
          ...
          set({ pipelineItems: pipeline, loading: false });
        } catch (error) {
          set({
            error:
      ...
      updatePost: async (id: string, updates: ContentPostUpdate) => {
        ...
        try {
          ...
          const updatedSelected =
            currentSelected?.id === id ? data : currentSelected;
      ...
      getFilteredPosts: () => {
        ...
        return posts.filter((post) => {
          ...
          if (filters.author && (post as any).author_id !== filters.author) {
            return false;
          }
          if (
            filters.searchTerm &&
          ...
          if (
            ...
            new Date(post.created_at) < new Date(filters.dateRange.start)
          ) {
            return false;
          }
          if (
            filters.dateRange.end &&
            new Date(post.created_at) > new Date(filters.dateRange.end)
          ) {
            return false;
          }
      ...
      getPipelineStats: () => {
        ...
        const byStatus = {
          ...
          scheduled: pipelineItems.filter((p) => p.status === "scheduled")
            .length,
          published: pipelineItems.filter((p) => p.status === "published")
            .length,
        ...
        Object.keys(byStatus).forEach((status) => {
          ...
          if (statusItems.length > 0) {
            const totalDays = statusItems.reduce(
              (sum, item) => sum + (item.days_in_status || 0),
              0
            );
            averageDaysInStatus[status as ContentStatus] = Math.round(
              totalDays / statusItems.length
            );
          }
        });
