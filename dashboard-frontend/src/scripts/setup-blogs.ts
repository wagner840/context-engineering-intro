...
const BLOGS_CONFIG: BlogConfig[] = [
  {
    ...
    description:
      "Blog especializado em inteligência artificial, desenvolvimento e tecnologia avançada",
  ...
  },
  {
    ...
    description:
      "Blog focado em estratégias de marketing digital, otimização e crescimento online",
...
// TODO: Descomentar quando blog_tags for adicionado aos tipos
/*
...
export async function setupBlogs() {
  ...
  for (const blogConfig of BLOGS_CONFIG) {
    try {
      ...
      if (existingBlog) {
        console.log(
          `✅ Blog ${blogConfig.name} já existe, atualizando configurações...`
        );
      ...
      console.log(
        `📥 Sincronizando posts do WordPress para ${blogConfig.name}...`
      );
      await syncWordPressToBlog(blog.id, blogConfig);

      // 4. Sincronizar categorias e tags
      console.log(
        `🏷️ Sincronizando categorias e tags para ${blogConfig.name}...`
      );
      ...
      const existingSlugs = new Set((existingTags.data || []).map(t => t.slug))
      ...
      if (tagsToCreate.length > 0) {
        await supabase.from('blog_tags').insert(tagsToCreate)
      }
      */
...
async function testWordPressConnection(
  ...
  try {
    const response = await fetch(
      `${blogConfig.wordpress_url}/wp-json/wp/v2/posts?per_page=1`
    );
...
async function syncWordPressToBlog(blogId: string, blogConfig: BlogConfig) {
  try {
    ...
    const response = await fetch(
      `${blogConfig.wordpress_url}/wp-json/wp/v2/posts?per_page=20&_embed`
    );
    ...
    for (const wpPost of wpPosts) {
      const postData = {
        ...
        word_count: wpPost.content.rendered
          .replace(/<[^>]*>/g, "")
          .split(/\s+/)
        ...
        seo_description: wpPost.excerpt.rendered
          .replace(/<[^>]*>/g, "")
          .substring(0, 160),
        focus_keyword: null,
      ...
      const { error } = await supabase.from("content_posts").upsert(postData, {
      ...
      if (error) {
        console.error(
          `Erro ao sincronizar post ${wpPost.title.rendered}:`,
          error
        );
      } else {
        console.log(
          `✅ Post sincronizado: ${wpPost.title.rendered.substring(0, 50)}...`
        );
      }
    }
  } catch (error) {
...
async function syncCategoriesAndTags(blogId: string, blogConfig: BlogConfig) {
  try {
    ...
    const categoriesResponse = await fetch(
      `${blogConfig.wordpress_url}/wp-json/wp/v2/categories?per_page=50`
    );
    if (categoriesResponse.ok) {
      ...
      for (const wpCategory of wpCategories) {
        ...
        await supabase.from("blog_categories").upsert(categoryData, {
    ...
    const tagsResponse = await fetch(
      `${blogConfig.wordpress_url}/wp-json/wp/v2/tags?per_page=50`
    );
    if (tagsResponse.ok) {
      ...
      for (const wpTag of wpTags) {
        ...
        await supabase.from("blog_tags").upsert(tagData, {
...
async function setupSampleKeywords(blogId: string, blogConfig: BlogConfig) {
  const sampleKeywords =
    blogConfig.niche === "Tecnologia e IA"
      ? [
          {
            blog_id: blogId,
  ...
  try {
    ...
    const newKeywords = sampleKeywords.filter(
      (k) => !existingKeywordsList.includes(k.keyword)
    );
